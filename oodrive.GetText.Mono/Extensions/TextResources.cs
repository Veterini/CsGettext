...
namespace Oodrive.GetText.Mono.Extensions
{
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="MonoPoResourceManager"/>s used in XAML via the <see cref="TextExtension"/> class must be registered
    /// with the <see cref="TextResources"/> class.
    /// If you change the current culture at runtime, call the <see cref="Invalidate()"/> method to refresh all instances
    /// of <see cref="TextExtension"/>.
    /// </remarks>
    ...
    public static class TextResources
    {
        ...
        private static readonly Dictionary<string, MonoPoResourceManager> PoResourceManagers = new Dictionary<string, MonoPoResourceManager>();

        public static void AddResourceManager(string key, MonoPoResourceManager manager)
        {
        ...
        public static MonoPoResourceManager GetResourceManager(string key)
        {
            key = key ?? DefaultResourceManagerKey ?? string.Empty;
            MonoPoResourceManager rm;
        ...
        private static readonly DependencyPropertyKey DependentObjectsPropertyKey =
            DependencyProperty.RegisterAttachedReadOnly("DependentObjects", typeof(List<IResourceDependent>), typeof(TextResources), new UIPropertyMetadata(null));
            ...
            where T : class
        {
            ...
            public int GetHashCode(WeakReference obj)
            {
                ...
                return target != null ? _valueComparer.GetHashCode(target) : 0;
            }
        }
    }
